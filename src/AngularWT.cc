...
{ 
  ...
  // using them in the angular decay rate for wrong-tagged events
...
Int_t AngularWT::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const
{
...
Double_t AngularWT::analyticalIntegral(Int_t code, const char* rangeName) const
{
  ...
  case 5:
    ...
    break;
  case 4:
    ret = 9./(32 * 3.14159265) * ( 4.0/9.0 + (1-Fl)/18.0*P1*cos(2*phi)*2*2 + (1-Fl)/9.0*P3*sin(2*phi)*2*2 ) * (2 * 2);
    break;
  case 3:
    L00 = ROOT::Math::assoc_legendre(0,0,ctL);
  ...
  case 2:
    ...
    ret = 9./(32 * 3.14159265) * ( (4.0/9.0)*K00 + (4.0*Fl/3.0-4.0/9.0)*K20 + (1-Fl)/18.0*P1*K22*cos(2*phi)*2 + (1-Fl)/9.0*P3*K22*sin(2*phi)*2 + (-2.0/3.0)*sqrt(Fl-Fl*Fl)*P5p*K21*cos(phi)*TMath::Pi()/4 + (-2.0/3.0)*sqrt(Fl-Fl*Fl)*P6p*K21*sin(phi)*TMath::Pi()/4 ) * (2);
    break;
  case 1:
    L00 = ROOT::Math::assoc_legendre(0,0,ctL);
    L10 = ROOT::Math::assoc_legendre(1,0,ctL);
    ...
    ret = 9./(32 * 3.14159265) * ( (4.0/9.0)*L00 + (2.0/9.0-2.0*Fl/3.0)*L20 + (-4.0/3.0)*P2*(1-Fl)*L10 + (1-Fl)/18.0*P1*L22*cos(2*phi)*2 + (1-Fl)/9.0*P3*L22*sin(2*phi)*2) * (2);
    break;
  }
