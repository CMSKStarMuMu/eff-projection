...
void test1Deff_asymm()
{
  ...
  // RooGenericPdf effPdf("effPdf","effPdf","2+mean*sin(x)",RooArgSet(x,mean));
  ...
  // cout<<"Left: "<<numData->sumEntries("x<0")<<" / "<<data->sumEntries("x<0")<<" = "<<numData->sumEntries("x<0")/data->sumEntries("x<0")<<endl;
  // cout<<"Rigt: "<<numData->sumEntries("x>0")<<" / "<<data->sumEntries("x>0")<<" = "<<numData->sumEntries("x>0")/data->sumEntries("x>0")<<endl;
  ...
  for (int iOrder=1; iOrder<=maxOrder; ++iOrder) {
    sinproj[iOrder] = numData->sumEntries(Form("sin(%i*x)>0",iOrder))/data->sumEntries(Form("sin(%i*x)>0",iOrder))
      - numData->sumEntries(Form("sin(%i*x)<0",iOrder))/data->sumEntries(Form("sin(%i*x)<0",iOrder));
    cosproj[iOrder] = numData->sumEntries(Form("cos(%i*x)>0",iOrder))/data->sumEntries(Form("cos(%i*x)>0",iOrder))
  ...
  for (int iOrder=1; iOrder<=maxOrder; ++iOrder) {
    sinFac[iOrder]->setVal(sinproj[iOrder]*TMath::Pi()/4);
    cosFac[iOrder]->setVal(cosproj[iOrder]*TMath::Pi()/4);
  }
